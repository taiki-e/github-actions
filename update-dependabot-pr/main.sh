...
pr_data=$(curl --proto '=https' --tlsv1.2 -fsSL --retry 10 -H "${HEADER}" "${pr_url}")
...
message=$(
    curl --proto '=https' --tlsv1.2 -fsSL --retry 10 "${commits_url}" \
...
curl --proto '=https' --tlsv1.2 -fsSL --retry 10 -X PATCH -H "${HEADER}" "${pr_url}" \
